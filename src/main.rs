...
fn main() -> NcResult<()> {
    let mut nc = FullMode::with_banners()?;
    let mut stdplane = nc.stdplane();

    welcome(&mut nc, stdplane)?;
    ...
    // underneath each to display a border without it interfering with coordinates.  Or maybe we could just
    // draw the borders on stdplane?
    ...
    // Plane current colour/style doesn't apply to putchar_yx?
    // board.on_styles(NCSTYLE_BOLD);
    // board.putchar_yx(12, 40, '@')?;
    // board.off_styles(NCSTYLE_BOLD);

    // test we can differentiate the above from a default char
    board.putchar_yx(12, 41, '@')?;
...
fn welcome(nc: &mut FullMode, stdplane: &mut NcPlane) -> NcResult<()> {
    let text = format!("Welcome to lartem!
...
",
    ...
    unsafe { ncplane_puttext(stdplane, 0, NCALIGN_LEFT, cstring![text], core::ptr::null_mut()) };
